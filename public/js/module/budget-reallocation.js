...
{   
    ...
    if(allow)
    {
        ...
        $.bootstrapGrowl("Reallocation more than remain, process cannot continue!", {
            type: "error"
        });
        $("#total"+x).val("");
...
{   
    if($("#coa_destination").val() == "")
    {
        
        $.bootstrapGrowl("Destination account cannot left blank!", {
            type: "error"
        });
    }
    else{
        ...
        $(coaDom).autocomplete({
            source: function( request, response ) {
                $.ajax({
                    url: baseUrl + "/finance/accounting/coa/rawdatabudget?term=" + $(coaDom).val() + "&year="+ $("#year").val() + "&month="+ $("#month").val() +"&org=" + $("#org_code").val() + "&coa_destination=" + $("#coa_destination").val(),
                    dataType: "json",
