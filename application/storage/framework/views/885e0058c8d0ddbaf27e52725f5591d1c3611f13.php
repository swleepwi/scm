...
<script>
    ...
    var gridMaster = $('#gridMaster').DataTable( {
        ajax: {
            type: "GET", 
            //url: "<?php echo e(url('/')); ?>/labelprint/datalist/"+start_date+"/"+end_date+"/0/0",
        },
        select: true,
		columnDefs: [  
			{ 
    ...
    {        
		...
		if(start_date == "" || end_date == "" )
		{
			...
			gridMaster.ajax.url("<?php echo e(url('/')); ?>/labelprint/datalist/"+start_date+"/"+end_date+"/"+po_number+"/"+order_number).load(null, false);
    ...
    {        		
		gridDetailLabel.ajax.url("<?php echo e(url('/')); ?>/labelprint/labeldatalist/"+po_number+"/"+seq_number).load();
		$("#span-type").html("Request");
		...
		columnDefs: [ 
			...
			{ 
                ...
                "render": function (dt, type, row ) {
                    ...
                    return row.CREATE_DATE.substr(0, 4)+'-'+row.CREATE_DATE.substr(4, 2)+'-'+row.CREATE_DATE.substr(6, 2); 
                }
            }
        ],	
        columns: [               
    ...
    {        		
		gridDetailSize.ajax.url("<?php echo e(url('/')); ?>/labelprint/sizedatalist/"+po_number+"/"+seq_number+"/"+uk_size).load();
		$("#span-type").html("Request");
	...
	function makeLabel()
	{
		if(obj.SelectedPO == "0")
		{
			...
			if(obj.hasLabel == "Y")
			{
				...
				$.ajax({
					type: "GET",
					url: "<?php echo e(url('/')); ?>/labelprint/makelabel/"+obj.SelectedPO+"/"+obj.SelectedPOSequence,
	...
	function printCheckedBarcode(){
		...
		if(totalSelected > 0)
		{
			...
			popUp("<?php echo e(url('/')); ?>/labelprint/printbylabels/"+po+"/"+po_sequence+"/"+labels, '450px', '500px', 'Print Preview')
		}
		else{
	...
	function printChecked(obj){
		...
		if(totalSelected > 0)
		{
			...
			var po =  $("#selectedPO").val();
			var po_sequence = $("#selectedPOSequence").val();
			popUp("<?php echo e(url('/')); ?>/labelprint/printbysizes/"+po+"/"+po_sequence+"/"+sizes, '450px', '500px', 'Print Preview')
		}
		else{
			$.bootstrapGrowl("No selected data!", {
				type: "error",offset: {from: 'top', amount: 250},align: 'center'
	...
	function printAll(){
		...
		if(totalRow > 0){			
			var po =  $("#selectedPO").val();
			var po_sequence = $("#selectedPOSequence").val();
			popUp("<?php echo e(url('/')); ?>/labelprint/printall/"+po+"/"+po_sequence, '450px', '500px', 'Print Preview');
		}
		else{
			$.bootstrapGrowl("No selected data!", {
				type: "error",offset: {from: 'top', amount: 250},align: 'center'
	...
	function printSingle(){
		...
		if(label !== ""){			
			...
			popUp("<?php echo e(url('/')); ?>/labelprint/printsingle/"+po+"/"+po_sequence+"/"+label, '450px', '500px', 'Print Preview');
		}
		else{
			$.bootstrapGrowl("No selected data!", {
				type: "error",offset: {from: 'top', amount: 250},align: 'center'
